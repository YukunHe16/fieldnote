...
# 3. OPTIONAL structural validation via the engine's XSD validator (roll-forward env — ISSUE-003).
#    Requires the minimax-docx marketplace plugin, which is NOT installed by default; the path
#    below often does not exist. Skip this step when it is missing — step 4 is the real gate.
MMDOCX_CLI=~/.claude/plugins/marketplaces/minimax-skills/skills/minimax-docx/scripts/dotnet/MiniMaxAIDocx.Cli
[ -d "$MMDOCX_CLI" ] && DOTNET_ROLL_FORWARD=Major dotnet run --project "$MMDOCX_CLI" \
