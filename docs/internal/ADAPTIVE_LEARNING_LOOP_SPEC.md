...
- 回路内出题（on-call 条件的 agent 会话挂载 `draft_practice_task`，replay 除外——回放工具集须与原始运行一致）：tutor 请求验证前先提交结构化题稿，同回合过三级质检——程序硬门（上限/答案泄漏，双脚本按字符权重同等）→ 新颖性硬门（对 session **已通过/已使用**题记 + 验证题面 + 目标的 Jaccard 查重，>0.6 拒；被拒草稿不入语料，否则修订重试会撞自己的废稿）→ LLM Evaluator（提示词附学习者已见文本；拒绝有效、基础设施错误 fail-open——超时也是 error，台账 reason 写「timed out」而不是 SDK 的「aborted by user」；后台预算 120s：思考型后台模型上实测一次真实判定 ~53s，低于 60s 的预算等于评审层永久空转且从指标上看不出来——上线后前三条 live 题记全是这么静默放行的；返回后对最新语料同步复检新颖性，落库前重验回路状态）。live/eval 由店面强制 `request_learning_verification` 携带通过题记，宿主把题面与 method 原样落库、同事务消费；通过未用项随该轮验证落地/轮次推进/incident 关闭作废；同 (incident, 轮次) 2 次**实质**被拒（novelty/evaluator，程序门不计）后解锁 prose 回退。全部尝试入 `learning_practice_items`（复习回访生成的标 `source='review'`）进研究导出；基线条件不挂载。
- eval-only tool compliance：run 开始/完成时由宿主比较 tutor-owed 状态签名；签名不动就写 `compliance_miss` 并为该签名排队至多一次 `【学习回路修复】`。repair run 不算 learner answer；完整推进记 `recovered`，仍欠任一步骤记 `gave_up`，runner 映射为 `tool_compliance_error` 并 fail-fast。live/飞书仍走原 2-turn watchdog，不受此恢复影响。
...
- 间隔回访开的是**新 incident**，不是复用旧的：衰减要有自己的时间戳、策略与结局，原来那次当时确实是解决了，改写它等于篡改历史。代价是同一个知识点在面板里会出现两条，看着像两个问题——所以 incident DTO 带 `reviewOf`（由 `learning_review_tasks.fired_run_id = incident.opened_run_id` 反查，无需新列），面板与单次报告都标出「第 N 次回访 · 同一个问题」。注意置信度图的 n 数的是**判定次数**不是问题数：回访产生的是一次独立判定，即使把两条 incident 合并，n 仍然是 2。
