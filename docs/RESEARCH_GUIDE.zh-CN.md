...
本文写给第一次接触 Fieldnote 的读者——计算机教育研究者、教师或助教。先讲清研究问题,然后用产品里**一条完整、未剪辑的真实会话**从头走到尾,用这条会话的截图逐一说明系统的每个部件如何对应到研究议程上。所有东西都在一台本地机器上运行,学生数据不出机器。(截图为英文界面;产品内置中英切换。)
