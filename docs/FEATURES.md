...
5. [对话式学习模式 / Adaptive learning conversation mode](#5-对话式学习模式--adaptive-learning-conversation-mode)
6. [可信专家协作 / Trusted specialist collaboration](#6-可信专家协作--trusted-specialist-collaboration)
7. [跨对话记忆 / Cross-conversation memory](#7-跨对话记忆--cross-conversation-memory)
8. [手册与通用自进化 / Handbook and general self-evolution](#8-手册与通用自进化--handbook-and-general-self-evolution)
9. [文件、工作区与成品货架 / Files, workspaces, and delivery shelf](#9-文件工作区与成品货架--files-workspaces-and-delivery-shelf)
10. [回放、重试与分支 / Replay, retry, and branches](#10-回放重试与分支--replay-retry-and-branches)
11. [飞书渠道 / Feishu channel](#11-飞书渠道--feishu-channel)
12. [数据、事件与 API / Data, events, and APIs](#12-数据事件与-api--data-events-and-apis)
13. [配置、部署与可观察性 / Configuration, deployment, and observability](#13-配置部署与可观察性--configuration-deployment-and-observability)
14. [安全与隐私边界 / Security and privacy boundaries](#14-安全与隐私边界--security-and-privacy-boundaries)
15. [明确的非目标与限制 / Explicit non-goals and limitations](#15-明确的非目标与限制--explicit-non-goals-and-limitations)
...
当前产品只有一个完整落地的教育业务方向：对话式学习模式，也就是本文档所说的教育值班回路。`local-operator`（界面中的“本地助手”）是它唯一的助手 Profile，同时提供通用文件、研究和多步骤任务底座。
...
The only fully implemented education domain today is the adaptive learning conversation mode—the educational on-call loop described in this document. `local-operator` is its single assistant profile and also serves as the general file, research, and multi-step task foundation.
...
学习回路与可信专家协作互相独立。一次运行可以同时使用学习模式和专家协作，但专家结果不会被自动当成学习成效；学习结果也不会自动发布成通用 Skill。

The learning loop and trusted specialist collaboration are independent. A run may use learning mode and specialists at the same time, but specialist output is not automatically treated as a learning outcome, and a learning outcome is not automatically published as a general Skill.
...
| 间隔复习回访 / Spaced review revisits | ✅ | ✅ | 到期时发进原对话 / Posted into the original conversation when due |
...
- 消息支持复制、系统分享、编辑、重试、回放、点赞和点踩；活动卡展示 Skill、MCP、工作区与嵌套专家活动的可读摘要。
...
- Messages support copy, system share, edit, retry, replay, thumbs-up, and thumbs-down. Activity cards present readable summaries of Skills, MCP tools, workspace work, and nested specialist activity.
...
| --- | --- | --- |
| `local-operator` | 唯一 Profile；界面中的“本地助手”，承载学习回路，并处理文件、研究和多步骤任务 | The single profile; the “local assistant” in the UI, host of the learning loop and of file, research, and multi-step work |

**中文**

- Profile 决定系统提示、允许的 Skills/MCP、专家角色、记忆作用域和能力面板。当前只有一个 Profile，但这一层结构保留，新的教育方向仍可作为新 Profile 接入。
- 随仓库分发的受控 plugin（文档技能、humanizer-zh）在这个 Profile 上可用。
- 本地助手只会在可信的 `inherit-user` 配置下沿用用户本机 Claude settings；`isolated` 模式不会加载用户级配置。

**English**

- A profile controls the system prompt, permitted Skills/MCP servers, specialist roles, memory scope, and capability panel. There is only one profile today, but the layer remains so that a new education direction can arrive as a new profile.
- The governed plugins shipped with the repository (document skills, humanizer-zh) are available on this profile.
...
## 5. 对话式学习模式 / Adaptive learning conversation mode

### 5.1 学习模式到底做什么 / What learning mode does
...
### 5.2 开启、暂停与建议 / Starting, pausing, and suggestions

**中文**

- 用户可从 Composer 旁的学习按钮主动开启，学习目标必填，topic 可选；之后可暂停、继续或结束。飞书用 `/learn 目标` 开启或刷新同一会话，`/learn off` 结束。
- **研究模式是唯一的复杂度开关**，位置在右上角 ⊞ 菜单第一项。关闭时（默认）界面里不出现参与者、学习主题、研究条件与数据导出，开启学习会话只问一句「你想学会什么」；打开后这四样一起出现。开关是全局的、本机的，关掉不影响任何记录——工单、干预、验证、练习题记、复习任务照常入库。
...
- **参与者（participant）轴**：顶栏「谁在学」可切换/新建参与者，与旁边的「用哪个助手」并排且各带标签——两者完全正交，一个是"谁在学"，一个是 Agent 配置档位。参与者切换器只在研究模式开启（或本机已有多位参与者）时出现。对话在创建时归属当前参与者；学习 Session、经验、讲法、策略修订、复习任务、练习题记全部按参与者反规范化落库并隔离——A 的经验绝不会调 B 的教学排序，复习与指标也互不串。侧栏会话列表只显示当前参与者的对话；非默认参与者与机主记忆**双向隔离**——既不进通用记忆抽取与能力自进化，其运行的提示词也不携带机主的记忆、手册与能力工件，memory 工具不挂载；标题整理与学习建议对参与者照常工作。学习面板（讲法/策略/指标/导出）锚定在**对话本人**而非全局切换器；回放继承原对话的参与者。飞书渠道在试点范围外，恒归默认参与者（HTTP 建会话同样强制）。研究导出支持 `?participantId=` 按人过滤，默认仍导出全库。
...
- **Research mode is the single complexity switch**, first item in the ⊞ menu. With it off (the default) the interface shows no participant, topic, research condition, or export, and starting a learning session asks one question: what you want to learn. Turning it on reveals all four together. The switch is global and local-only, and turning it off records nothing less — incidents, interventions, verifications, practice drafts, and review tasks are all still written.
...
- **Participant axis**: the header's "Who's learning" control switches/creates participants, sitting beside "Assistant" with both labelled — two orthogonal questions, one about the person and one about the agent configuration. The participant control appears only when research mode is on (or the machine already has more than one participant). Conversations belong to the current participant at creation; learning sessions, experiences, invented approaches, policy revisions, review tasks, and practice items are all denormalized and isolated per participant — one person's experiences never reorder another's teaching, and reviews/metrics never cross. The sidebar lists only the current participant's conversations; non-default participants are isolated from the owner's memory in BOTH directions — excluded from extraction and capability self-evolution, and their runs carry none of the owner's memories, playbooks, or capability artifacts (the memory tools are not mounted), while auto-titling and learning suggestions keep working for them. The learning panel (approaches/policies/metrics/export) anchors on the conversation's own person, never the global switcher; replays inherit the source conversation's participant. The Feishu channel is outside the pilot and always belongs to the default participant. The research export accepts `?participantId=` for a per-person slice and still defaults to the whole study.
...
### 5.3 困难、教学干预与验证 / Incidents, interventions, and verification
...
### 5.5 受控教学策略演进 / Governed teaching-strategy evolution
...
### 5.6 稳定演示与真实 Agent 演示 / Stable and Real Agent demos
...
### 5.7 学习 MCP 的权限边界 / Learning MCP authority boundary

**中文**

Active Session 才加载以下内部工具：`open_learning_incident`、`record_learning_intervention`、`request_learning_verification`、`propose_learning_outcome`、`escalate_learning_incident`。on-call 条件的 agent 会话（replay 数据集除外——回放要求工具集与原始运行一致）额外加载 `draft_practice_task`（见 5.7c）。宿主校验状态顺序、Run/消息归属和轮次。

模型不能确认最终学习结果、不能直接写策略统计、不能启用 policy，也不能把内部工具正文显示成学生回答。学习者确认与 policy 审核始终是宿主侧、用户可见的操作。

**English**

Only an active session loads these internal tools: `open_learning_incident`, `record_learning_intervention`, `request_learning_verification`, `propose_learning_outcome`, and `escalate_learning_incident`. Agent sessions in the on-call condition (except the replay dataset, whose toolset must match the original run) additionally load `draft_practice_task` (see 5.7c). The host validates state ordering, run/message ownership, and round limits.
...
### 5.7c 回路内出题与三级质检 / In-loop practice generation & three-tier review
...
### 5.9 升级交接报告 / Escalation handoff reports
...
### 5.10 讲法自发明 / Invented teaching approaches
...
## 6. 可信专家协作 / Trusted specialist collaboration

### 6.1 受控专家与真实执行 / Governed specialists and real execution

**中文**

当前不再内置固定的领域专家角色。可委派的专家来自受控自进化产出、并经人工审核启用的子代理（见 8.2）。专家只在复杂工作需要时由主 Agent 委派；简单问题不强制拆分，用户也可要求跳过。

每次委派都会启动独立、真实的 Claude Agent SDK child query，而不是向虚构 Mailbox 写一封“内部邮件”。Child 的正文和工具活动会嵌套流入当前 Run；专家不能继续嵌套委派。

**English**

There are no built-in domain specialist roles today. Delegable specialists are evolved subagents produced by governed self-evolution and enabled through human review (see 8.2). The main Agent delegates only when complex work benefits from it; simple questions are not forcibly split, and users may ask it to skip delegation.
...
### 6.2 生命周期与结构化结果 / Lifecycle and structured results
...
### 6.4 呈现方式与学习模式关系 / Presentation and relationship to learning mode
...
## 7. 跨对话记忆 / Cross-conversation memory

### 7.1 记忆类型与来源 / Types and sources
...
### 7.2 读取、整理与用户控制 / Retrieval, maintenance, and user control
...
## 8. 手册与通用自进化 / Handbook and general self-evolution
...
### 8.2 通用能力自进化 / General capability evolution
...
### 8.3 与学习策略演进的区别 / Difference from learning-policy evolution
...
## 9. 文件、工作区与成品货架 / Files, workspaces, and delivery shelf

### 9.1 上传与统一输入 Manifest / Upload and unified input manifest
...
### 9.2 分支、历史与 Replay 文件语义 / Branch, history, and Replay semantics
...
### 9.3 成品呈交与 Delivery Shelf / Presentation and Delivery Shelf
...
## 10. 回放、重试与分支 / Replay, retry, and branches

### 10.1 编辑与重试 / Edit and retry
...
### 10.2 可复核 Replay / Auditable Replay

**中文**

完成的 Run 会尝试创建 snapshot，冻结原始 prompt 与 supplements、Profile、工作区副本、playbooks、记忆、evolved artifacts、输入 Manifest 和学习上下文。Snapshot 成功创建后，Replay 在新对话和新工作区恢复这些值，不回退到当前记忆或当前 overlay；若冻结阶段的工作区复制失败，该 Run 仍可能完成但之后没有可用 snapshot。
...
A completed run attempts to create a snapshot that freezes the original prompt and supplements, profile, workspace copy, playbooks, memories, evolved artifacts, input manifest, and learning context. Once the snapshot succeeds, Replay restores those values in a new conversation and workspace without falling back to current memory or the current overlay. If workspace copying fails during freezing, the run may still complete without an available snapshot.
...
- 命令包括 `/help`、`/new`/`/clear`、`/agent`、`/agent local`、`/stop`、`/continue`、`/guide text` 和 `/learn 目标`（`/learn off` 结束）。
...
- Commands include `/help`, `/new`/`/clear`, `/agent`, `/agent local`, `/stop`, `/continue`, `/guide text`, and `/learn <goal>` (`/learn off` ends it).
...
## 12. 数据、事件与 API / Data, events, and APIs

### 12.1 本地持久化 / Local persistence

**中文**

- SQLite 默认位于 `data/agent.db`，保存对话、消息、Run、事件、SDK sessions、附件元数据、记忆、协作任务、学习状态、策略与配置。
- 每个对话使用 `data/workspaces/<conversation-id>` 工作区；附件、Replay 快照和 runtime plugins 使用各自受控目录。
...
- SQLite defaults to `data/agent.db` and stores conversations, messages, runs, events, SDK sessions, attachment metadata, memory, collaboration tasks, learning state, policies, and configuration.
- Each conversation uses a workspace at `data/workspaces/<conversation-id>`. Attachments, Replay snapshots, and runtime plugins use separate governed locations.
...
### 12.2 主要 API 家族 / Main API families
...
### 12.3 公共契约与事件 / Contracts and events
...
## 13. 配置、部署与可观察性 / Configuration, deployment, and observability

### 13.1 启动与模型配置 / Startup and model configuration
...
### 13.2 可观察性与测试 / Observability and testing
...
### 14.1 已有防护 / Implemented protections
...
### 14.2 必须理解的边界 / Boundaries users must understand
...
- 教学反馈和外部检索到的信息可能出错或过期；重要决定必须由用户检查官方来源和最终文件。
...
- Educational feedback and externally retrieved information can be wrong or stale. Users must verify important decisions against official sources and final artifacts.

## 15. 明确的非目标与限制 / Explicit non-goals and limitations
...
- 可信专家只是受人审启用的受控子代理；它不是开放式 Agent Marketplace、A2A 网络或多机器人组织。
- 不自动付款、发邮件、向外部人员发送消息或替用户做不可逆决定。
...
- 当前唯一完整教育业务方向是对话式学习模式；未来 topic 和研究合作可以复用学习、文件、协作、Replay 与演进基础，但尚未因此变成已交付产品。
...
- Trusted specialists are governed subagents enabled through human review. This is not an open Agent Marketplace, A2A network, or multi-bot organization.
- The product does not automatically make payments, send email, message external people, or make irreversible decisions for the user.
...
- The adaptive learning conversation mode remains the only complete education domain. Future topics and research collaborations may reuse the learning, file, collaboration, Replay, and evolution foundation, but that does not make them shipped products today.
