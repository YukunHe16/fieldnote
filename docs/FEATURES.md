...
- **参与者（participant）轴**：顶栏「谁在学」可切换/新建参与者，与旁边的「用哪个助手」并排且各带标签——两者完全正交，一个是"谁在学"，一个是 Agent 配置档位。参与者切换器只在研究模式开启（或本机已有多位参与者）时出现。对话在创建时归属当前参与者；学习 Session、经验、讲法、策略修订、复习任务、练习题记全部按参与者反规范化落库并隔离——A 的经验绝不会调 B 的教学排序，复习与指标也互不串。侧栏会话列表只显示当前参与者的对话；非默认参与者与机主记忆**双向隔离**——既不进通用记忆抽取与能力自进化，其运行的提示词也不携带机主的记忆、手册、领域卡与能力工件，memory 工具不挂载；标题整理与学习建议对参与者照常工作。学习面板（讲法/策略/指标/导出）锚定在**对话本人**而非全局切换器；回放继承原对话的参与者。飞书渠道在试点范围外，恒归默认参与者（HTTP 建会话同样强制）。研究导出支持 `?participantId=` 按人过滤，默认仍导出全库。
